...
import ExpandHorizontalIcon from "../Icon/ExpandHorizontal";
...
export default function SessionItem({ session }: SessionItemProps) {
        ...
        setSessions((sessions) => {
          return sessions.map((sessionItem) => {
            if (sessionItem.id === session.id) {
              const oldIndex = sessionItem.materials.findIndex(
                (material) => material.id === active.id
              );
              const newIndex = sessionItem.materials.findIndex(
                (material) => material.id === over?.id
              );

              const updatedMaterials = arrayMove(
                sessionItem.materials,
                oldIndex,
                newIndex
              );
  ...
  const { editSessionName, setSelectedSessionId, deleteSession } = useSession();
          ...
          {edit ? (
            <>
              <CustomInput
                ...
                className="!w-max"
              />
              <Button type="fill" className="!p-2 !h-auto" onClick={handleEdit}>
               Save
              </Button>
            </>
          ) : (
            <>
              <h2 className="font-medium text-2xl font-medium">
                {session.name}{" "}
              </h2>
          ...
          >
            {session.materials.length ? (
              ...
              <div className="text-center">
                Session don't have any material yet
              </div>
            )}
          </SortableContext>
        </DndContext>
